...
with tab_comps:
    ...
    with st.expander("🧭 Peer seçim metodolojisi, kiminle karşılaştırmalıyız ve neden", expanded=False):
        st.markdown(peers_data.get("peer_methodology", "Henüz eklenmedi."))
        narrow = peers_data.get("narrow_peers", [])
        if narrow:
            st.markdown("**Dar peer grubu (gerçek boru/tüp uzmanları):**")
            st.dataframe(narrow, use_container_width=True, hide_index=True)
        else:
            st.info(
                "Dar peer grubu henüz boş. EquityRT'de Tenaris, Vallourec, Jindal SAW gibi şirketleri arayıp "
                "verilerini gönderince `data/cfa_peers.json` içindeki `narrow_peers` listesine işlenir."
            )
    ...
    if not peers_list:
        ...
        rows = [
            {"Şirket": p["name"], "Ülke": p.get("ulke", "bilinmiyor"), "Piyasa Değeri (mn $)": p["market_cap_mn_usd"],
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Peer ortalama (mean) P/E", f"{mean_pe:.2f}" if mean_pe else "yok")
        c2.metric("Peer medyan (median) P/E", f"{median_pe:.2f}" if median_pe else "yok")
        c3.metric("Peer ortalama P/BV", f"{mean_pbv:.2f}" if mean_pbv else "yok")
        c4.metric("Peer medyan P/BV", f"{median_pbv:.2f}" if median_pbv else "yok")
